...
void assert_vec2f(CT_Vec2f *v, float x, float y) {
  CT_CHECK(ct_deltaeqf(v->x, x, EPS) && ct_deltaeqf(v->y, y, EPS),
           "[%f,%f] != [%f,%f]\n", v->x, v->y, x, y);
fail:
  return;
}
...
void test_vec2f() {
  CT_INFO("-------- test_vec2 --------");
  CT_MPool pool;
  if (!ct_mpool_init(&pool, 16, sizeof(CT_Vec2f))) {
...
void test_vec3f() {
  CT_INFO("-------- test_vec3 --------");
  CT_MPool pool;
  if (!ct_mpool_init(&pool, 16, sizeof(CT_Vec3f))) {
